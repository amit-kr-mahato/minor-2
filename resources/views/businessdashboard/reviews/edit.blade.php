...
@section('content')
  <div class="ml-64 w-full p-8 shadow-md rounded">
    ...
    <form action="{{ route('businessdashboard.reviews.update', $review->id) }}" method="POST">
    @csrf
    @method('PUT')

    <div class="mb-4">
      <label class="block text-gray-700 font-bold mb-2">User</label>
      <input type="text" disabled value="{{  $review->user?->name ?? 'Unknown User'}}"
      class="bg-gray-100 border border-gray-300 rounded w-full py-2 px-3">
    </div>

    <div class="mb-4">
      <label class="block text-gray-700 font-bold mb-2">Business</label>
      <input type="text" disabled value="{{ $review->business?->name ?? 'Unknown Business' }}"
      class="bg-gray-100 border border-gray-300 rounded w-full py-2 px-3">
    </div>
    ...
    <div class="mb-4">
      ...
      class="border border-gray-300 rounded w-full py-2 px-3">{{ old('review', $review->review) }}</textarea>
    </div>
  ...
  <div class="flex justify-end">
    ...
    class="mr-3 bg-red-600 py-2 px-4 rounded font-bold text-white hover:underline">Cancel</a>
